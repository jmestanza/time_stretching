...
def get_fundamental_frequency(audio, is_frame_speech,K,L,fs,config_dict, w_type = "hanning", show_demo = False, figsize=(10, 12)):
    ...
    for i, x1_and_x2 in enumerate(split_audio):
        ...
        if is_frame_speech[indexes[i]]:
            ...
            f0_hat_in_samples = get_f0_from_autocorr(autocorrelation, show_demo, config_dict)#np.argmax(autocorrelation)
            if f0_hat_in_samples != None:
        ...
        if show_demo:
            show_demo = False
    return new_indexes, f0s_in_samples




def get_f0_from_autocorr(x,show_demo, config_dict):
    
    peaks, info = find_peaks(x, height=config_dict["height"],
                                distance = config_dict["distance"],
                                prominence= config_dict["prominence"],
